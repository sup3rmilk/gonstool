...
class Uploader(object):
    ...
    def upload_video(self, item, filename):
        ...
        if user["downloads"] == 0:
            self.api.sendMessage(str(item["userid"]), "Bot: Thank you for using this tool,please consider sharing this account with your friends and leave any feedback/issues under the feedback post !")
            self.counter += 1
...
class InboxHandler(object):
    def __init__(self, API, config, uploader, d_uploader):
        ...
        self.admins = ["download_tool", "download_tool1"]
